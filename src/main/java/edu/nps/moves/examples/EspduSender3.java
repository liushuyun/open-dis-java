 ...
 * @author DMcG
 */
public class EspduSender3 implements Runnable{
    ...
    public EspduSender3(Map<String, String> receivedSignal_ValueMap) {
        this.thread = new Thread(this);//创建新线程

        this.target = receivedSignal_ValueMap.get("target");
    }
...
//        System.out.println(firePduJsonString);
        // 使用GsonBuilder来定制Gson实例
        Gson gson = new GsonBuilder()
                .setPrettyPrinting() // 设置Gson输出格式化为易读的格式
                .create();
        FirePdu firePdu = gson.fromJson(firePduJsonString, FirePdu.class);

        firePdu.setMunitionID(new EntityID());
//        firePdu.setRangeToTarget(0.0f);
//        firePdu.setVelocity(new Vector3Float());
        firePdu.setLocationInWorldCoordinates(new Vector3Double());

//        firePdu = new FirePdu();
//        EntityID fesid = new EntityID();
//        fesid.setSite(1);
//        fesid.setApplication(1);
//        fesid.setEntity(23);
//        firePdu.setFiringEntityID(fesid);
//        EntityID teID = new EntityID();
//        teID.setSite(1);
//        teID.setApplication(1);
//        teID.setEntity(3);
//        firePdu.setTargetEntityID(teID);
        con.send(firePdu);
        System.out.println(gson.toJson(firePdu));
...
//        }
    ...
    public void run() {
        //        DisConnection con = new DisConnection(DEFAULT_MULTICAST_GROUP, DIS_PORT);
        DisConnection con = null;
        try {
            con = new DisConnection("225.1.2.3", 9393);
        } catch (IOException e) {
